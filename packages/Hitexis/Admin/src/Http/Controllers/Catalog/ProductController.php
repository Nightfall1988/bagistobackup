...
namespace Hitexis\Admin\Http\Controllers\Catalog;
...
use Hitexis\Product\Helpers\ProductType;
...
use Webkul\Admin\Http\Controllers\Catalog\ProductController as WebkulProductController;
...
class ProductController extends Controller
{
    ...
    public function __construct(
        protected HitexisProductRepository $hitexisProductRepository,
    ...
    public function index()
    {
        ...
        return view('hitexis-admin::catalog.products.index', compact('families'));
    }
    ...
    public function store()
    {
        ...
        $product = $this->hitexisProductRepository->create(request()->only([
            'type',
            'attribute_family_id',
            'sku',
            'super_attributes',
    ...
    {
        $product = $this->hitexisProductRepository->findOrFail($id);

        $inventorySources = $this->inventorySourceRepository->findWhere(['status' => self::ACTIVE_STATUS]);

        return view('hitexis-admin::catalog.products.edit', compact('product', 'inventorySources'));
    }
    ...
    public function update(ProductForm $request, int $id)
    {
        ...
        $product = $this->hitexisProductRepository->update(request()->all(), $id);
    ...
    {
        try {
            $product = $this->hitexisProductRepository->copy($id);
        } catch (\Exception $e) {
    ...
    public function destroy(int $id): JsonResponse
    {
        try {
            ...
            $this->hitexisProductRepository->delete($id);
    ...
    public function massDestroy(MassDestroyRequest $massDestroyRequest): JsonResponse
    {
        ...
        try {
            foreach ($productIds as $productId) {
                $product = $this->hitexisProductRepository->find($productId);

                if (isset($product)) {
                    Event::dispatch('catalog.product.delete.before', $productId);

                    $this->hitexisProductRepository->delete($productId);
    ...
    public function massUpdate(MassUpdateRequest $massUpdateRequest): JsonResponse
    {
        ...
        foreach ($productIds as $productId) {
            ...
            $product = $this->hitexisProductRepository->update([
    ...
    public function search()
    {
        ...
        $products = $this->hitexisProductRepository->searchFromDatabase([
            'status'               => null,
